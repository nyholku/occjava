...
%rename(Geom_Geometry) handle<Geom_Geometry>;
%rename(Geom_Curve) handle<Geom_Curve>;
...
%typemap(javaout) Handle_Geom_Geometry, Handle_Geom_Geometry* {
    long cPtr = $jnicall;
...
%typemap(javaout) Handle_Geom_Curve, Handle_Geom_Curve* {
    long cPtr = $jnicall;
    return ($javaclassname)Standard_Transient.downcastHandle(cPtr, $javaclassname.class);
}
